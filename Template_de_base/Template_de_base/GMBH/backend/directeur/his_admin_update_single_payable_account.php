...
<?php
	...
	include('assets/inc/config.php');
		if(isset($_POST['update_acc']))
		{
			...
			$query="UPDATE  his_accounts SET nom=?, marque=?, service=?, date_mise_en_servicet=? WHERE code_équipement = ?";
			$stmt = $mysqli->prepare($query);
			$rc=$stmt->bind_param('sssss', $code_équipement, $nom, $marque, $service, $date_mise_en_service);
			$stmt->execute();
			/*
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <?php
                ...
                $ret="SELECT  * FROM his_accounts WHERE code_équipement=?";
                $stmt= $mysqli->prepare($ret) ;
                $stmt->bind_param('i',$code_équipementr);
                $stmt->execute() ;//ok
                $res=$stmt->get_result();
                //$cnt=1;
